...
exports['default'] = function () {
  return {
    ...
    reportTaskStart: function reportTaskStart(startTime, userAgents, testCount) {
      ...
      this.SuiteID = Number(process.env.SUITE_ID) || 0;
    ...
    publishResultToTestrail: function publishResultToTestrail() {
      ...
      api.closePlan(this.PlanID,function (err, response){
           if (err === 'null') {
              _this3.newline().write(_this3.chalk.blue('---------Error at Add result -----')).newline().write(err);
           }else {
               _this3.newline().write('------------------------------------------------------').newline().write(_this3.chalk.green('Test Plan close successfully'));
           }
      });
    },
    ...
    getPlanID: function getPlanID(api) {
      ...
      api.getPlans(this.ProjectID, function (err, response, plan) {
        ...
        if (err !== 'null') {
          ...
          for (var index in plan.plans) {
            if (plan.plans[index].name === _this5.PlanName) {
              _this5.newline().write(_this5.chalk.blue.bold('Plan name(id) ')).write(_this5.chalk.yellow(plan.plans[index].name + '(' + plan.plans[index].id + ')'));
      ...
      return api.getSuites(this.ProjectID, function (err, response, suites) {
        if (err !== 'null') {
          ...
          if (suites.length === 0) {
            ...
            var suite = suites.find(suite => suite.id === _this7.SuiteID) || suites[0];
            _this7.newline().write(_this7.chalk.blue.bold('Suite name(id) ')).write(_this7.chalk.yellow(suite.name + '(' + suite.id + ')'));
            _this7.SuiteID = suite.id;
          }
        } else {
